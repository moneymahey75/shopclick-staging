    ...
    if (parentAccount) {
      ...
      if (sponsorProfile) {
        ...
        if (!sponsorUser?.tu_is_active || !sponsorIsVerified || (!isFiveSpinRegistration && !sponsorUser?.tu_registration_paid)) {
          return new Response(
            JSON.stringify({ success: false, error: isFiveSpinRegistration ? 'Parent A/C must be active and email or mobile verified' : 'Parent A/C is not active/verified or registration-paid' }),
            {
              status: 400,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            }
